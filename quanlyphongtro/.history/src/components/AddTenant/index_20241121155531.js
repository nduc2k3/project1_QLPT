...
<<<<<<< Tabnine <<<<<<<
    ...
    /**//+
     * This function represents the main component for adding a new tenant.//+
     * It handles the logic for adding tenant information, service usage, and member details.//+
     * //+
     * @returns {JSX.Element} - The JSX element for the AddTenant component.//+
     *///+
    function AddTenant() {//+
        const [activeMenu, setActiveMenu] = useState('thongTin');//+
        const [tenantData, setTenantData] = useState({});//+
...
//+
        ...
        try {//-
            ...
            try {//+
                const response = await axios.post('https://qlpt.onrender.com/api/tenant', tenantData);//+
                // Display server message on successful save//+
                if (response.data && response.data.message) {//+
                    alert(response.data.message);//+
                }//+
                navigate('/tenants');//+
            } catch (error) {//+
                // Display server message on save error//+
                if (error.response && error.response.data && error.response.data.message) {//+
                    alert(error.response.data.message);//+
                }//+
                console.error(error);//+
...
>>>>>>> Tabnine >>>>>>>// {"conversationId":"9c7b8710-9513-4f77-85fd-457b79cf0bae","source":"instruct"}
